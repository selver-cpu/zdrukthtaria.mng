...
class DokumentetProjektiController extends Controller
{
    ...
    {
        $request->validate([
            'dokument' => 'required|file|mimes:jpg,jpeg,png,pdf,doc,docx,xls,xlsx,zip,stl,step,skp,dwg,rar,txt,csv,html,htm,css,js,json,xml,md,log,ini,conf,env,ppt,pptx,rtf,odt,ods,odp|max:20480', // max 20MB
            'pershkrimi' => 'nullable|string|max:255',
            'kategoria' => 'nullable|string|in:vizatim,dimension,material,3d_model,excel,tjeter',
        ]);
    ...
    public function update(Request $request, string $id)
    {
        ...
        $request->validate([
            'pershkrimi' => 'nullable|string|max:255',
            'kategoria' => 'nullable|string|in:vizatim,dimension,material,3d_model,excel,foto,tjeter',
        ]);
    ...
    public function destroy(Projektet $projekt, string $id)
    {
        try {
            ...
            // Thjeshtë kontrollin e autorizimit - lejojmë fshirjen e dokumentit
            // Fshi skedarin fizik
            if (Storage::disk('public')->exists($dokument->rruga_skedarit)) {
